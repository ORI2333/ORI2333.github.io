...
```yaml
image:
  favicon: /blog/images/favicon.png
```
...
additional_injections:
  after_footer: "<script>window.__WALINE_SERVER='你的Waline地址';window.__COMMENTS_ENABLED=true;</script>"
```
...
- 主题的 `visit_count` 只在 `post` 位点启用，`index` 位点未启用。
- 或者未配置 Waline 服务地址（`window.__WALINE_SERVER` 仍是占位值）。

处理：

1. 在 `_config.kratos-rebirth.yml` 中将 `visit_count.enable_at` 同时包含 `index` 与 `post`。
2. 确认 `additional_injections.after_footer` 里包含真实 Waline 地址，例如：

```yaml
additional_injections:
  after_footer: "<script>window.__WALINE_SERVER='https://你的-waline-域名';window.__COMMENTS_ENABLED=false;</script>"
```

说明：

- 阅读量统计可以独立于评论开关运行；即使评论关闭（`__COMMENTS_ENABLED=false`），阅读量仍可显示。
- 首页列表展示的是各文章路径的阅读量，只有配置了有效 Waline 服务地址才会正常返回数据。
